  ...
  try {
    ...
    for (const [key, item] of Object.entries(promptConfig)) {
      try {
        ...
        if (item.field) {
          const fieldData = data[item.field]
          const filteredData: Record<string, any> = {}
          for (const [nestedKey, nestedValue] of Object.entries(fieldData || {})) {
            if (!nestedKey.startsWith('占位')) {
              filteredData[nestedKey] = nestedValue
            }
          }
